...
void QuickHull3D::build(vector<double> coords, int nump){
    ...
    if (nump < 4){
        printf("Less than 4 points, break");
        return;
    }
...
QuickHull3D::QuickHull3D(vector<double> coords, int len){
    newFaces = new FaceList();
    unclaimed = new VertexList();
    claimed = new VertexList();
    explicitTolerance = AUTOMATIC_TOLERANCE;
...
void QuickHull3D::setHull(vector<double> coords, int nump, vector<vector<int> > faceIndices, int numf){
    initBuffers(nump);
    setPoints(coords, nump);
    computeMaxAndMin();
