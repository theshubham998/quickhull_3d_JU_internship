...
void Facet::computeNormalAndCentroid(){
    ...
    if (numV != numVerts){
        printf("Error in computeNormalAndCentroid");
    }
}
...
void Facet::checkConsistency(){
    ...
    if (numVerts < 3){
        printf("numVerts < 3, Error in checkConsistency, Stop");
    }
    do{
        HalfEdge *hedgeOpp = hedge->getOpposite();
        if (hedgeOpp == NULL){
            printf("hedgeOpp is NULL,Error in checkConsistency, Stop");
        }
        else if (hedgeOpp->getOpposite() != hedge){
            printf("hedge's Opp's opp != hedge, Error in checkConsistency, Stop");

        }
        if (hedgeOpp->head() != hedge->tail() ||
                hedge->head() != hedgeOpp->tail()){
            printf("hedgeOpp.head != hedge.tail, Error in checkConsistency, Stop");
            HalfEdge *he = he0;
            do {
                printf("Index: %d", he->head()->index);
                he = he->next;
            }while (he != he0);
        }
        Facet *oppFace = hedgeOpp->facet;
        if (oppFace == NULL){
            printf("oppFace is NULL, Error in checkConsistency, Stop");

        }
        else if (oppFace->mark == DELETED){
            printf("oppFace is deleted, Error in checkConsistency, Stop");
            HalfEdge *he = he0;
            do {
                printf("Index: %d", he->head()->index);
                he = he->next;
    ...
    if (numV != numVerts){
        printf("numV(%d) != numVerts(%d), Error in checkConsistency, Stop", numV, numVerts);
    }
}
